...
class wizard_report(osv.osv_memory):
    ...
    _columns = {
        'afr_id': fields.many2one('pawn.afr', 'Custom Report', help='If you have already set a Custom Report, Select it Here.'),
    ...
    def onchange_afr_id(self, cr, uid, ids, afr_id, context=None):
        ...
        if not afr_id:
            return res
        afr_brw = self.pool.get('pawn.afr').browse(cr, uid, afr_id, context=context)
        res['value'].update({
