...
class account_balance(report_sxw.rml_parse):
    ...
    # kittiu
    def get_branch(self, form):
        pawn_shop_obj = self.pool.get('pawn.shop')
        pawn_shop_id = form['pawn_shop_id'] and form['pawn_shop_id'][0] or False
        if pawn_shop_id:
            pawn_shop = pawn_shop_obj.browse(self.cr, self.uid, form['pawn_shop_id'][0])
            return pawn_shop.name
        return False
    # --
    ...
    def get_informe_text(self, form):
        ...
        if afr_id:
            name = self.pool.get('pawn.afr').browse(self.cr, self.uid, afr_id).name
    ...
    def _get_partner_balance(self, account, init_period, ctx=None):
        ...
        if account['type'] in ('other', 'liquidity', 'receivable', 'payable'):
            ...
            # kittiu
            WHERE_PAWN_SHOP = ''
            if ctx.get('pawn_shop_id', False):
                WHERE_PAWN_SHOP += " AND aml.pawn_shop_id = %s" % (ctx.get('pawn_shop_id'),)
            if ctx.get('profit_center', False):
                WHERE_PAWN_SHOP += " AND aml.profit_center = %s" % (ctx.get('profit_center'),)
            # --
            ...
            query_init = sql_query % ('SUM(aml.debit) AS init_dr',
                                      ...
                                      '0.0 AS bal_dr',
                                      '0.0 AS bal_cr',
                                      WHERE + WHERE_POSTED + WHERE_PAWN_SHOP)
    ...
    def _get_analytic_ledger(self, account, ctx={}):
        ...
        if account['type'] in ('other', 'liquidity', 'receivable', 'payable'):
            ...
            # kittiu
            if ctx.get('pawn_shop_id', False):
                where += " AND aml.pawn_shop_id = %s" % (ctx.get('pawn_shop_id'),)
            if ctx.get('profit_center', False):
                where += " AND aml.profit_center = %s" % (ctx.get('profit_center'),)
            # --
    ...
    def lines(self, form, level=0):
        ...
        """
        # kittiu
        self.context.update({'pawn_shop_id': form['pawn_shop_id'] and form['pawn_shop_id'][0] or False,
...
report_sxw.report_sxw('report.pawn.afr.1cols',
                      'wizard.report',
                      'pawn_account_financial_report/report/balance_full.rml',
                      parser=account_balance,
                      header=False)

report_sxw.report_sxw('report.pawn.afr.2cols',
                      'wizard.report',
                      'pawn_account_financial_report/report/balance_full_2_cols.rml',
                      parser=account_balance,
                      header=False)

report_sxw.report_sxw('report.pawn.afr.4cols',
                      'wizard.report',
                      'pawn_account_financial_report/report/balance_full_4_cols.rml',
                      parser=account_balance,
                      header=False)

report_sxw.report_sxw('report.pawn.afr.analytic.ledger',
                      'wizard.report',
                      'pawn_account_financial_report/report/balance_full_4_cols_analytic_ledger.rml',
                      parser=account_balance,
                      header=False)

report_sxw.report_sxw('report.pawn.afr.partner.balance',
                      'wizard.report',
                      'pawn_account_financial_report/report/balance_full_4_cols_partner_balance.rml',
                      parser=account_balance,
                      header=False)

report_sxw.report_sxw('report.pawn.afr.journal.ledger',
                      'wizard.report',
                      'pawn_account_financial_report/report/balance_full_4_cols_journal_ledger.rml',
                      parser=account_balance,
                      header=False)

report_sxw.report_sxw('report.pawn.afr.5cols',
                      'wizard.report',
                      'pawn_account_financial_report/report/balance_full_5_cols.rml',
                      parser=account_balance,
                      header=False)

report_sxw.report_sxw('report.pawn.afr.qtrcols',
                      'wizard.report',
                      'pawn_account_financial_report/report/balance_full_qtr_cols.rml',
                      parser=account_balance,
                      header=False)

report_sxw.report_sxw('report.pawn.afr.13cols',
                      'wizard.report',
                      'pawn_account_financial_report/report/balance_full_13_cols.rml',
                      parser=account_balance,
                      header=False)
