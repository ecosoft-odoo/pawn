...
class aged_trial_report_pawn(aged_trial_report):

    def set_context(self, objects, data, ids, report_type=None):
        partner_ids = data['form'].get('partner_ids', False)
        if partner_ids:
            res = super(aged_trial_report_pawn, self).set_context(objects, data, partner_ids, report_type)
            self.query += ' AND l.partner_id in (%s) ' % ','.join(str(x) for x in partner_ids)
        else:
            res = super(aged_trial_report_pawn, self).set_context(objects, data, ids, report_type)
        return res
...
report_sxw.report_sxw('report.account.aged_trial_balance_pawn', 'res.partner',
        'addons/account/report/account_aged_partner_balance.rml', parser=aged_trial_report_pawn,
